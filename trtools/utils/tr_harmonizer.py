...
def InferVCFType(vcffile: cyvcf2.VCF, vcftype: Union[str, VcfTypes] = "auto") \
    ...
    if _IsBeagleType(user_supplied_type) and _BeagleType() in possible_vcf_types:
        if user_supplied_type == VcfTypes.beagle_hipstr:
            try:
                for key in 'START', 'END', 'PERIOD':
                    vcffile.get_header_type(key)
            except KeyError:
                raise TypeError("This Beagle VCF does not contain the INFO fields "
                                "that would identify it as containing HipSTR calls."
                                " Did you forget to copy those fields over? See: "
                                "https://trtools.readthedocs.io/en/latest/BEAGLE.html")
            else:
                return VcfTypes.beagle_hipstr
        else:
...
def _HarmonizeHipSTRRecord(vcfrecord: cyvcf2.Variant,
    ...
    if (vcfrecord.INFO.get('START') is None
        ...
        raise TypeError(("Locus {}:{} is not a HipSTR record, "
                         ...
                         "imputed HipSTR record, make sure to filter SNP loci from "
                         "the VCF and copy the original INFO fields over. See: "
                         "https://trtools.readthedocs.io/en/latest/BEAGLE.html )")
...
def _GetBeagleAPQuality(vcfrecord: cyvcf2.Variant) -> np.ndarray:
    ...
    # intentionally treat the record as unphased regardless of its phasing
    # this algorithm currently only works for diploid organisms
    # and that there are no partial calls or no calls
    # TODO: relax these assumptions
    arr = vcfrecord.genotype.array()
    assert arr.shape[1] == 3
    idx_gts = arr[:, :-1]
    assert ~np.any(idx_gts == -1)
    assert ~np.any(idx_gts == -2)

    aps = []
    _range = np.arange(idx_gts.shape[0])
    for copy in (1,2):
        ap = vcfrecord.format(f'AP{copy}')
        # append the reference probability to the front of the list
        ref_prob = np.maximum(0, 1 - np.sum(ap, axis=1))
        ref_prob = ref_prob.reshape(-1, 1)
        ap = np.concatenate((ref_prob, ap), axis=1)
        aps.append(ap)
    probs = aps[0][_range, idx_gts[:, 0]]*aps[1][_range, idx_gts[:, 1]]

    # since we are computing unphased qualities
    # need to add AP1[A1]*AP2[A2] + AP1[A2]*AP2[A1] for het calls
    het_calls = idx_gts[:, 0] != idx_gts[:, 1]
    probs[het_calls] += aps[0][het_calls, idx_gts[het_calls, 1]] * \
...
class TRRecord:
    ...
    def UniqueStringGenotypeMapping(self) -> Dict[int, int]:
        ...
        Dict[int, int]
            ...
            string alleles for this variant. For most records
    ...
    def GetQualityScores(self) -> np.ndarray:
        ...
        if self.quality_field:
            qvals = self.format[self.quality_field]
            assert len(qvals.shape) == 2 and qvals.shape[1] == 1
            return qvals[:, 0]
        else:
