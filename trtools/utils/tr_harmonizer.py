...
class TRRecord:
    ...
    def GetDosages(self, 
        ...
        if (dosagetype in [TRDosageTypes.beagleap, TRDosageTypes.beagleap_norm]) and \
            (self.vcfrecord.FORMAT.get("AP1", None) is None or self.vcfrecord.FORMAT.get("AP2", None) is None):
                raise ValueError(
        ...
        if dosagetype in [TRDosageTypes.beagleap, TRDosageTypes.beagleap_norm]:
            ...
            ap1 = self.vcfrecord.FORMAT.get("AP1", None)
            ref1 = np.clip(1-np.sum(ap1, axis=1), 0, 1) # If neg due to rounding, cutoff at 0
            ap2 = self.vcfrecord.FORMAT.get("AP2", None)
