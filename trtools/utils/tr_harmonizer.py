...
def InferVCFType(vcffile: cyvcf2.VCF, vcftype: Union[str, VcfTypes] = "auto") -> VcfTypes:
    ...
    if len(possible_vcf_types) > 1:
        ...
        if vcftype != "auto" and user_supplied_type in possible_vcf_types:
...
def _HarmonizeGangSTRRecord(vcfrecord: cyvcf2.Variant):
    ...
    TRRecord
    """
    if vcfrecord.INFO.get('RU') is not None:
...
class TRRecord:
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 quality_score_transform: Optional[Callable[Any, float]] = None):
    ...
    def _CheckRecord(self):
        ...
        """
        if self.alt_alleles is not None:
            assert len(self.alt_alleles) == len(self.vcfrecord.ALT)
        else:
            assert len(self.alt_allele_lengths) == len(self.vcfrecord.ALT)
