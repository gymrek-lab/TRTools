...
if __name__ == "tr_harmonizer":
    sys.path.insert(0, os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "strtools", "utils")
    )
    import utils
else:
...
class TRRecordHarmonizer:
    ...
    def _InferVCFType(self):
        ...
        popSTR:
           ...
           LT field gives only repeat counts.
...
class TRRecord:
    ...
    def GetGenotypeCounts(self,
        ...
        genotype_counts = {}
        for sample in self:
            if len(samplelist) > 0 and sample.sample not in samplelist:
                continue
            if sample.called:
                if uselength:
    ...
    def GetMaxAllele(self,
        ...
        if not self.HasFullStringGenotypes() or not fullgenotypes:
            ...
            # trimmed alleles
            return max_len + len(self.full_alleles[0]) - len(self.ref_allele)
    ...
    def __str__(self):
        ...
        if len(self.alt_alleles) == 0:
            ...
            string += ",".join(str(length) for length
